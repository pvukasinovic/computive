...
module requantize #(
    ...
    input rst_n,
    input valid_in,
    input signed [ACC_W-1:0] acc [0:PARALLELISM-1],
    // Scalar requant inputs (used when PER_CHANNEL=0)
    /* verilator lint_off UNUSEDSIGNAL */
    input [ACC_W-1:0] scale,  // M (unsigned)
    input [5:0] shift,  // Right-shift amount
    input signed [DATA_W-1:0] zero_point,  // Output zero point
    // Per-channel requant buses (used when PER_CHANNEL=1)
    input [PARALLELISM*ACC_W-1:0] scale_bus,  // packed per-lane M
    input [PARALLELISM*6-1:0] shift_bus,  // packed per-lane shift
    input [PARALLELISM*DATA_W-1:0] zp_bus,  // packed per-lane zp
