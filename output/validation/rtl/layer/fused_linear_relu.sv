...
module fused_linear_relu #(
    ...
    input [15:0] output_dim,  // Used by accelerator_top for addressing
    /* verilator lint_on UNUSEDSIGNAL */
    input [15:0] num_tiles,
    input has_relu,
    input [15:0] weight_base,
    input [15:0] bias_base,
    input [ACC_W-1:0] requant_scale,
    input [5:0] requant_shift,
    input signed [DATA_W-1:0] requant_zp,

    // Weight SRAM interface
    output logic [15:0] weight_addr,
    /* verilator lint_off UNUSEDSIGNAL */
    input [PARALLELISM*DATA_W-1:0] weight_rdata,  // Routed to MAC array in accelerator_top
    /* verilator lint_on UNUSEDSIGNAL */

    // Bias SRAM interface
    output logic [15:0] bias_addr,
    input [PARALLELISM*ACC_W-1:0] bias_rdata,

    // Activation read interface (from ping-pong input bank)
    output logic [$clog2(ACT_DEPTH)-1:0] act_rd_addr,
    input [ACT_WIDTH-1:0] act_rd_data,
    ...
    input signed [DATA_W-1:0] mac_output_data [0:PARALLELISM-1],
    /* verilator lint_off UNUSEDSIGNAL */
    input mac_output_valid,  // FSM uses mac_done instead
    /* verilator lint_on UNUSEDSIGNAL */
    input mac_done,
