...
module layer_norm_unit #(
    ...
    // Configuration
    input  logic [15:0] feature_dim,
    input  logic [31:0] epsilon_fixed,   // epsilon in fixed-point (unused in LUT path)

    // Activation input (read)
    output logic [$clog2(MAX_FEAT_DIM)-1:0] act_rd_addr,
    input  logic signed [DATA_W-1:0]        act_rd_data,
    ...
    input  logic signed [DATA_W-1:0]        scale_data,
    output logic [$clog2(MAX_FEAT_DIM)-1:0] bias_addr,
    input  logic signed [DATA_W-1:0]        bias_data,

    // Reciprocal-sqrt LUT ROM interface
    output logic [DATA_W-1:0]    rsqrt_lut_addr,
    input  logic [RSQRT_LUT_W-1:0] rsqrt_lut_data
);
