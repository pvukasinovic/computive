...
module mac_array #(
    ...
    input  logic [PARALLELISM*DATA_W-1:0]      weight_data,
    /* verilator lint_off UNUSEDSIGNAL */
    input  logic                               weight_valid,  // AXI-spec port, timing driven by FSM
    ...
    input  logic signed [ACC_W-1:0]            bias_data,
    input  logic                               bias_valid,
    input  logic [$clog2(PARALLELISM)-1:0]     bias_idx,

    // Requantization parameters (scalar — used when PER_CHANNEL=0)
    input  logic [ACC_W-1:0]                   scale,
    input  logic [5:0]                         shift,
    input  logic signed [DATA_W-1:0]           zero_point,

    // Per-channel requant buses (used when PER_CHANNEL=1)
    /* verilator lint_off UNUSEDSIGNAL */
    input  logic [PARALLELISM*ACC_W-1:0]       scale_bus,
    input  logic [PARALLELISM*6-1:0]           shift_bus,
    input  logic [PARALLELISM*DATA_W-1:0]      zp_bus,
    /* verilator lint_on UNUSEDSIGNAL */
